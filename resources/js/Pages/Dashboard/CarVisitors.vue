...
function applyFilters() {
  router.get(route('dashboard.visitors.with-vehicle'), {
    ...
    <div class="space-y-5">
      <div class="flex items-center justify-between">
        <div>
          ...
          <h1 class="text-2xl font-bold text-foreground mt-1">Visitors With Vehicle</h1>
        </div>
      </div>
      ...
      <DataTable :columns="columns" :rows="visits.data">
        ...
        <template #vehicle="{ row }">
          <div v-if="row.vehicle">
            <p class="text-sm text-foreground">{{ row.vehicle.plate_number }}</p>
            <p class="text-xs text-muted-foreground">{{ [row.vehicle.make, row.vehicle.model, row.vehicle.color].filter(Boolean).join(' ') || '-' }}</p>
          </div>
