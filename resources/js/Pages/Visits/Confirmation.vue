...
<template>
  <AppLayout>
    <div class="max-w-lg mx-auto space-y-5">
      <Link :href="route('visits.index')" class="inline-flex items-center gap-1.5 text-sm text-muted-foreground hover:text-foreground transition-colors">
        <ArrowLeft class="w-4 h-4" /> Back to visits
      </Link>

      <Card class="p-6 space-y-5 text-center">
        <div>
          <h1 class="text-xl font-bold text-foreground">Visit Registered</h1>
        ...
        <div v-if="visit.qr_token" class="flex flex-col items-center gap-3">
          ...
          <p class="text-sm text-muted-foreground">
            Have the visitor show this code at the gate for a quick check-in.
          </p>
        </div>
        ...
        <Button as="a" :href="route('visits.index')" variant="outline">Back to Visits</Button>
      </Card>
    </div>
  </AppLayout>
</template>
